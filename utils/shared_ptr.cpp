...
private:
    T * ptr = nullptr;
    uint * refCount = nullptr; // # shared_ptr objects owning the same underlying pointer
...
public: 
    ...
    my_shared_ptr(const my_shared_ptr & obj){ // copy contructor
    ...
    my_shared_ptr(my_shared_ptr && dyingObj) 
    {
        ...
        dyingObj.ptr = dyingObj.refCount = nullptr; // mmm, I have transferred ownership, but dyingObj is still alive
    }
    ...
    // we also need object dereferencing through overloaded operators in our class 
...
// Another templated version with[] i.e. <T[]> is also added to support the creation of arrays.
...
public:
	...
	my_shared_ptr(const my_shared_ptr & obj) = delete; // copy constructor is deleted
	my_shared_ptr& operator=(const my_shared_ptr & obj) = delete; // copy assignment is deleted
